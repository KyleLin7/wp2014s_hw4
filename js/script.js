...
FB.getLoginStatus(function(response) {
 ...
 //      document.getElementsByTagName('body').innerHTML = ""
    ...
    FB.api('/me', function (response) {
    	var str="It's "+response.name+"'s Facebook Profile Picture:";
        $('#name').html(str);
    });
    ...
    function handleMouseMove(e){//滑鼠移動的event
      ...
      img3.src = $('#typo').val();
		...
		//	canvas.height = profileIMG.height;
			...
			ctx.drawImage(profileIMG,canMouseX-profileIMG.width/2,canMouseY);//從XY軸0，0值開始畫如profileimg
			ctx.drawImage(img3,canMouseX-img3.width/2,canMouseY+profileIMG.height); //劃入img3，並根據你的滑鼠游標移動，你可以自行更換想要移動的圖層，數值會因XY軸向有所不同
			var inputedText = $('#inputed').val();//抓取頁面inputed ID的內容
			ctx.fillStyle = "black"; //字體顏色
			ctx.font='20px "微軟正黑體"'; //字體大小和字形
			ctx.fillText(inputedText,canMouseX-img3.width/2+70,canMouseY+profileIMG.height+40); //字體也可以依據滑鼠游標移動，所輸入的值可自行調整，若不想移動輸入的字體，可以把它改成（inputedText,0,0)X Y軸 0，0的位置
      }
    }
